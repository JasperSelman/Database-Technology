...
In the current project we want to verify the claims based in the paper by simulating the behavior of the \emph{smooth scan}n. First of all, the penalties for the index scan and full scan will be estimated. These estimates will be used to calculate the cost and thereby the execution time for the full and index scan for queries with different values of selectivity. Also different properties of the database will included and compared, such as clustered versus unclustered.
Thereafter, we will simulate the behavior of the \emph{switch scan}, which will switch on set cardinality estimate.
If there is time left, we will conclude with the simulation of the \emph{smooth scan}.
