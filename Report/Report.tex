...
In the current project we want to verify the claims based in the paper by simulating the behavior of the smooth scan. First of all, the penalties for the index scan and full scan will be estimated. These estimates will be used to calculate the cost and thereby the execution time for the full and index scan for queries with different values of selectivity. Also different properties of the database will included and compared, such as clustered versus unclustered.
Thereafter, we will simulate the behavior of the \emph{switch scan}, which will switch on set cardinality estimate. This estimate will be %comparable to the value set by Borovica et al. \cite{smoothscan}, who used a cardinality of 15K tuples out of 400M.
If there is time left, we will conclude with the simulation of the smooth scan.
